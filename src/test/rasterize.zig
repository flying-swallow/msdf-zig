...
fn runCase(allocator: std.mem.Allocator, io: std.Io, font: []const u8, case: Case) !void {
    ...
    const bearing_x = result.glyph_data.bearing_x;
    const bearing_y = result.glyph_data.bearing_y;
    ...
    for (target_sizes) |ts| {
        ...
        const sdf_left_em = bearing_x - px_range_em / 2.0;
        const sdf_top_em = bearing_y + px_range_em / 2.0;
        ...
        for (0..box_h) |y| {
            for (0..box_w) |x| {
                ...
                const bounds_left = bearing_x;
                const bounds_bottom = bearing_y - (f64i(h) - f64i(case.opts.px_range)) / f64i(case.opts.px_size);
                
                const translate_x = -bounds_left + px_range_em / 2.0;
                const translate_y = -bounds_bottom + px_range_em / 2.0;
